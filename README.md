...
-  「画像生成AIを使って表紙を作成する」という旨を先方に通達済み
...
init_editorの1行目に存在するminifyed jsonの中身（minifiedしているのはトークン節約のため）。ここで編集者として必要なスキルを鍛えている。何か編集者としての挙動が変だったり、調整したい場合は、このファイルから編集し、yamlからjsonに変換すると良い。
